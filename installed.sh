sudo apt -y install ffmpeg audacity nmap powertop aircrack-ng openssl wifite ssh meld tp-smapi-dkms libssl-ocaml-dev libcurl4-openssl-dev  libnetfilter-queue-dev libusb-1.0-0-dev libpcap-dev ttf-mscorefonts-installer gnome-shell-timer xsane g++ cmake apt-transport-https ca-certificates  curl  gnupg-agent software-properties-common  ansible zsh lib32z1  xboxdrv john git gcc make libpcap-dev telnet tmux redis-server pinta  imagemagick adb conky fonts-takao-gothic fonts-takao fonts-takao-mincho  fonts-takao-pgothic fonts-ipafont-gothic hexedit ghex unrar unzip bchunk python3-pip htop vim thinkfan gimp inkscape gnome-clocks  terminator filezilla vlc thunar gnome-tweaks openvpn ubuntu-restricted-extras tlp arp-scan chromium-browser virtualbox virtualbox-ext-pack wxhexeditor hexedit pinta obs-studio fonts-font-awesome gnome-shell-extensions mpg123 jpegoptim ack-grep speedtest-cli
# set up thinkfan. Read Fedora19 thinkfan manual.
